...
/**
 * Represents a 2D pose with position and orientation in a Cartesian coordinate
 * system. Yaw is in radians, counter-clockwise. +x is right-face direction, +y
 * is front-face direction The rest of the standardization is up to the end-user
 * to decide, but is highly recommended ro stick to meters.
 *
 * If you know CFrames in Roblox, this library should be very easy to
 * understand: https://create.roblox.com/docs/reference/engine/datatypes/CFrame
 */
class TwoDPose {
public:
  ...
  /**
   * Constructs a TwoDPose object representing a position and orientation in 2D
   * space.
   * @param x The x-coordinate of the pose, where positive values indicate
   * rightward movement.
   * @param y The y-coordinate of the pose, where positive values indicate
   * forward movement.
   * @param yaw The orientation of the pose in radians, where positive values
  ...
  // Returns a TwoDPose that represents the delta change applied to the *=,
  // relative to the first TwoDPose
  /**
   * Calculates the transformation required to go from this pose to another
   * pose. This can be thought of as the relative movement and rotation from
   * this pose to the specified pose.
   ...
   * This is analogous to calculating the pose of an object relative to a
   * reference frame defined by this pose.
  ...
  /**
   * Transforms a pose from the local coordinate space of this pose back into
   * the global coordinate space. This is the inverse operation of
   * toObjectSpace, effectively recalculating the global coordinates of a pose
   * that has been transformed into this pose's local space. It re-applies the
   * global positioning and orientation of this pose to the local pose.
   * @param other The pose in this object's local coordinate space to transform
  ...
  /**
   * Transforms given position and orientation coordinates into the coordinate
   * space of this pose. Useful for recalibrating or 'taring' sensors relative
   * to a current pose.
  ...
  /**
   * Returns a pose with the same x and y, but with the yaw turned to face the
   * designated x and y
  ...
  /**
   * Returns a string representation of the pose with the coordinates rounded to
   * a specified number of decimal places. If decimal_places is -1, no rounding
   * is applied.
   * @param decimal_places The number of decimal places to round to, or -1 for
   * no rounding.
  ...
  /**
   * Returns a string representation of the pose with the coordinates rounded to
   * a specified number of decimal places. This variant follows pose standard
   * for the T265 If decimal_places is -1, no rounding is applied.
   * @param decimal_places The number of decimal places to round to, or -1 for
   * no rounding.
   * @return A string in the format "x y yaw".
   */

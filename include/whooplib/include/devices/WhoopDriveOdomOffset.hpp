...
public:
   ...
   * the odometry trackers. Visual Representation of Odom Location:
   * https://imgur.com/x8ObCIG
   * @param odom_unit A pointer to the odometry unit
   * @param x_offset The x offset of the odom unit from the center of the robot
   * (positive implies a shift right from the center of the robot).
   * @param y_offset The y offset of the odom unit from the center of the robot
   * (positive implies a shift forward from the center of the robot).
   */
  WhoopDriveOdomOffset(WhoopDriveOdomUnit *odom_unit, double x_offset,
                       double y_offset);
...
public: // This is one of the ONLY exceptions to be public, as another module
   ...
   */
  void __step_down(); // This steps down to the Odom Unit (meant to be managed
                      // by drivetrain or fusion object)

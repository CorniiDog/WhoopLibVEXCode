...
protected:
  double yaw_offset =
      0; // Offset to adjust the yaw reading to account for initial orientation.
...
public:
  ...
  /**
   * Constructor to initialize an inertial sensor on a specified port with an
   * option to reverse the readings.
   * @param port The port number where the inertial sensor is connected.
   * @param reversed enum to set if the sensor readings should be reversed
   * (true) or not (false).
   */
  WhoopInertial(std::int32_t port, double correction_multiplier);

  /**
   * Retrieves the current yaw value with any applied offsets, in degrees by
   * default.
  ...
  /**
   * Retrieves the current roll value with any applied offsets, in degrees by
   * default.
  ...
  /**
   * Retrieves the current pitch value with any applied offsets, in degrees by
   * default.
  ...
  /**
   * Initiates calibration of the inertial sensor. This process may take a few
   * seconds.
   */
  void calibrate();

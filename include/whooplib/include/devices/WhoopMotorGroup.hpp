...
/**
 * Manages a group of WhoopMotors, allowing synchronized control over multiple
 * motors.
 */
class WhoopMotorGroup {
private:
  /**
   * Applies a specified member function to all motors in the group with a
  ...
  /**
   * Applies a specified member function to all motors in the group without
   * arguments.
   * @param func Pointer to the member function to be applied.
   */
...
protected:
  // Upon initialization
  std::vector<WhoopMotor *>
      whoop_motors; // Vector of pointers to WhoopMotor objects.

  double gear_ratio = 1; // Gear ratio for scaling motor output, default is 1.
  double wheel_diameter = to_meters(4); // Gear ratio for scaling motor output,
                                        // default is 0.1016 meters, or 4 inches
...
public:
  ...
  /**
   * Constructor that initializes a motor group with a vector of motors, with
   * addition to gear ratio
   * @param ratio i.e. motor on 32 tooth powering the 64 tooth: ratio
   * = 32.0/64.0 = 0.5
   * @param motors Vector of pointers to initialized WhoopMotors.
   */
   ...
   * @param motors Vector of pointers to initialized WhoopMotors.
   */
  WhoopMotorGroup(double ratio, double diameter_meters,
                  std::vector<WhoopMotor *> motors);
  ...
  // Motor commands
  void spin(double volts); // Commands all motors to spin at a specified
                           // voltage. -12.0 -> 0 -> 12.0, with 0 being stopped.
  void spin_unit(double unit); // Commands all motors to spin at a unit -1 -> 0
                               // -> 1, with 0 being stopped.
  void spin_percentage(
      double percentage); // Commands all motors to spin at a specified
                          // percentage -100 -> 0 -> 100, with 0 being stopped.
   ...
   */
  void set_gear_ratio_mult(double ratio); // motor on 32 tooth powering the 64
  ...
  // Receiving rotation
  // Note: If 3 or more motors are in a motor group, the motor group would get
  // the average of n-1 motors (excluding the outlier motor furthest from
  // average).
  double
  get_rotation(); // Returns the average rotation across all motors in degrees.
  double get_rotation_degrees();   // Returns the average rotation across all
                                   // motors in degrees.
  double get_rotation_radians();   // Returns the average rotation across all
                                   // motors in radians.
  double get_rotation_rotations(); // Returns the average rotation across all
                                   // motors in full rotations.
  ...
  /**
   * Gets the distance traveled by the motor group in meters (use case would be
   * for a drivetrain)
   * @returns
   */
  ...
  void tare(double degrees); // Resets the encoder count for all motors to a
                             // specified degree value.
  void tare_degrees(double degrees); // Resets the encoder count for all motors
                                     // to a specified degree value.
  void tare_radians(double radians); // Resets the encoder count for all motors
                                     // to a specified radian value.
  void
  tare_rotations(double rotations); // Resets the encoder count for all motors
                                    // to a specified number of full rotations.

  /**
   * Tares the wheels to set meters, if wheel diameter and gear ratio is set
   * appropriately.

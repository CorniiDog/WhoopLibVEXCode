 ...
 * @param confidence (non-zero with vision system) If for the vision system, it
 * would output a value between [0,1] where 1 is the highest confidence. 0.3 or
 * higher means it tracks, and 0.5 or higher means the tracking is good.
 */
struct Pose {
  double x = 0, y = 0, z = 0;
  double pitch = 0, yaw = 0, roll = 0;
  double confidence =
      0; // If for the vision system, it would output a value between 0 and 1
         // where 1 is the highest confidence. 0.3 or higher means it tracks,
         // and 0.5 or higher means the tracking is good.
  Pose() {}
  Pose(double x, double y, double z, double pitch, double yaw, double roll,
       double confidence = 0)
 ...
 */
enum tare_remaining_0 { do_tare = true, dont_tare = false };
...
/**
 * Manages vision processing for robotics, handling pose estimation and
 * transformations based on vision sensor input.
 */
class WhoopVision {
public:
...
protected:
  ...
  RobotVisionOffset
      *robot_offset; // Offset configuration for vision adjustments.

  std::vector<std::function<void(Pose)>>
      callback_functions; // Callbacks registered for incoming messages.

  /**
   * Transforms the raw pose data based on the current configuration and tare
   * settings.
   * @param apply_delta Flag to determine if delta adjustments should be
   * applied.
   */
...
public:
  ...
  /**
   * Constructor for initializing the vision system with a specific
   * configuration.
   ...
   */
  WhoopVision(RobotVisionOffset *robotOffset, BufferNode *bufferSystem,
              std::string pose_stream);

  // Taring (resetting) methods for the pose estimation.
  void tare(double x, double y, double z, double pitch, double yaw,
            double roll);

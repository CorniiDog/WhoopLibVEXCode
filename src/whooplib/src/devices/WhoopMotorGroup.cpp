...
WhoopMotorGroup::WhoopMotorGroup(double ratio, double diameter_meters,
                                 std::vector<WhoopMotor *> motors)
    : WhoopMotorGroup(ratio, motors) {
  set_wheel_diameter(diameter_meters);
}

void WhoopMotorGroup::apply_to_all(void (WhoopMotor::*func)(double),
                                   double value) {
...
void WhoopMotorGroup::set_wheel_diameter(double diameter_meters) {
  if (diameter_meters <= 0) {
    throw std::invalid_argument(
        "Wheel diameter must be positive and non-zero.");
  }
  wheel_diameter = diameter_meters;
  wheel_circumference = circumference_from_diameter(wheel_diameter);
}

void WhoopMotorGroup::stop_hold() { apply_to_all(&WhoopMotor::stop_hold); }

void WhoopMotorGroup::stop_brake() { apply_to_all(&WhoopMotor::stop_brake); }

void WhoopMotorGroup::stop_coast() { apply_to_all(&WhoopMotor::stop_coast); }
...
double WhoopMotorGroup::get_rotation() {
  ...
  if (whoop_motors.size() > 2) {
    auto max_it = std::max_element(
        rotations.begin(), rotations.end(), [&avg](double a, double b) {
...
double WhoopMotorGroup::get_rotation_degrees() { return get_rotation(); }
...
double WhoopMotorGroup::get_velocity() {
  ...
  if (whoop_motors.size() > 2) {
    auto max_it = std::max_element(
        velocities.begin(), velocities.end(), [&avg](double a, double b) {
          return std::abs(a - avg) < std::abs(b - avg);
        });
    total -= *max_it;
    avg = total / (whoop_motors.size() - 1);
...
double WhoopMotorGroup::get_velocity_deg_s() { return get_velocity(); }
double WhoopMotorGroup::get_velocity_rad_s() { return to_rad(get_velocity()); }
...
void WhoopMotorGroup::tare() { apply_to_all(&WhoopMotor::tare, 0); }

void WhoopMotorGroup::tare(double degrees) {
  apply_to_all(&WhoopMotor::tare, degrees / gear_ratio);
}

void WhoopMotorGroup::tare_degrees(double degrees) { tare(degrees); }

void WhoopMotorGroup::tare_rotations(double rotations) {
  tare(rotations * 360.0);
}

void WhoopMotorGroup::tare_radians(double radians) { tare(to_deg(radians)); }

void WhoopMotorGroup::tare_meters(double meters) {
  if (wheel_diameter <= 0) {
    throw std::invalid_argument(
        "Wheel diameter must be set and positive to tare by meters.");
  }

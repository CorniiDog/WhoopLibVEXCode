...
WhoopRotation::WhoopRotation(std::int32_t port)
    : WhoopRotation(port, reversed::no_reverse) {}

WhoopRotation::WhoopRotation(std::int32_t port, reversed reversed)
    :
#if USE_VEXCODE
      vex::rotation(port, reversed) {
}
#else
...
#if USE_VEXCODE
  return vex::rotation::position(rotationUnits::deg) + pos_offset;
#else
  return pros::Rotation::get_position() /
         100.0; // Position in centidegrees, so we convert
#endif
}
...
double WhoopRotation::get_rotation_degrees() { return this->get_rotation(); }
...
#if USE_VEXCODE
  return vex::rotation::velocity(velocityUnits::dps);
#else
  return pros::Rotation::get_velocity() /
         100.0; // Convert from centidegrees/s to degrees/s
#endif
}
double WhoopRotation::get_velocity_deg_s() { return get_velocity(); }
double WhoopRotation::get_velocity_rad_s() { return to_rad(get_velocity()); }
...
void WhoopRotation::tare(double degrees) {
  pos_offset = degrees;
#if USE_VEXCODE
  vex::rotation::resetPosition();
#else
  pros::Rotation::reset_position();
#endif
}

void WhoopRotation::tare() { this->tare(0); }

void WhoopRotation::tare_degrees(double degrees) { this->tare(degrees); }
